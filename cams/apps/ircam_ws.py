...
def layout():
    ...
    def build_img_container(index:int):
        return html.Div(
        [
            html.Img(id=f"apps-camera-img-{index}", className="apps-camera-img"),
            html.Div(html.H3("IR Camera View"), id="apps-camera-title-{index}", className="apps-camera-title"),
            html.Div(
                [
                    html.Span(""),
                    html.Span("-none-", id=f"apps-camera-info-{index}", className="apps-camera-info"),
                ],
                id=f"apps-camera-info-box-{index}",className="apps-camera-info-box",
            ),
            WebSocket(id=f"apps-camera-ws-{index}", url=_ws_url),
        ],
